...
impl OutlinePanel {
    ...
    fn is_foldable(&self, entry: EntryRef, cx: &mut ViewContext<Self>) -> bool {
        self.picker
            .update(cx, |picker, cx| picker.delegate.is_foldable(entry, cx))
    }
...
impl OutlinePanelDelegate {
    ...
    fn is_foldable(&self, entry: EntryRef, cx: &mut ViewContext<Picker<Self>>) -> bool {
    ...
    fn autoscroll(&mut self, cx: &mut ViewContext<Picker<Self>>) {
        if let Some(selected_entry) = self.selected_entry.clone() {
            ...
            if let Some(index) = index {
                self.picker.read(cx).scroll_to_item_index(index);
                cx.notify();
            }
        }
    }
                ...
                .update(&mut cx, |picker, cx| {
                    ...
                    if prefetch {
                        let range = if picker.delegate.last_visible_range.is_empty() {
                            0..(picker.entries_with_depths(cx).len() / 4).min(50)
                        } else {
